...
function rowToTrade(row:any):Trade{return {
  id:row.id,date:row.trade_date,accountId:row.account_id||'',market:row.market||'Futures',
  instrument:row.instrument,instrumentType:row.instrument_type||'Futures',direction:row.direction,
  optionType:row.option_type||'',strikePrice:row.strike_price==null?undefined:Number(row.strike_price),
  expiryDate:row.expiry_date||'',strategy:row.strategy,entry:Number(row.entry_price),exit:Number(row.exit_price),
  quantity:Number(row.quantity),lotSize:Number(row.lot_size||1),lots:Number(row.lots||1),
  stopLoss:row.stop_loss==null?undefined:Number(row.stop_loss),target:row.target==null?undefined:Number(row.target),
  brokerage:Number(row.brokerage||0),exchangeCharges:Number(row.exchange_charges||0),taxes:Number(row.taxes||0),
  slippage:Number(row.slippage||0),charges:Number(row.charges||0),grossPnl:Number(row.gross_pnl||0),
  pnl:Number(row.net_pnl),followedRules:Boolean(row.followed_rules),emotion:row.emotion||'Focused',
  notes:row.notes||'',screenshotUrl:row.screenshot_url||''
}}
function tradeToRow(t:Trade,userId:string){return {
  id:t.id,user_id:userId,account_id:t.accountId||null,trade_date:t.date,market:t.market,instrument:t.instrument,
  instrument_type:t.instrumentType,direction:t.direction,option_type:t.optionType||null,strike_price:t.strikePrice||null,
  expiry_date:t.expiryDate||null,strategy:t.strategy,entry_price:t.entry,exit_price:t.exit,quantity:t.quantity,
  lot_size:t.lotSize,lots:t.lots,stop_loss:t.stopLoss||null,target:t.target||null,brokerage:t.brokerage,
  exchange_charges:t.exchangeCharges,taxes:t.taxes,slippage:t.slippage,charges:t.charges,gross_pnl:t.grossPnl,
  net_pnl:t.pnl,followed_rules:t.followedRules,emotion:t.emotion,notes:t.notes,screenshot_url:t.screenshotUrl||null
}}
function rowToAccount(row:any):TradingAccount{return {id:row.id,name:row.name,broker:row.broker,accountType:row.account_type,startingCapital:Number(row.starting_capital),isDefault:Boolean(row.is_default)}}
function accountToRow(a:TradingAccount,userId:string){return {id:a.id,user_id:userId,name:a.name,broker:a.broker,account_type:a.accountType,starting_capital:a.startingCapital,is_default:a.isDefault}}
function rowToCapital(row:any):CapitalEntry{return {id:row.id,date:row.entry_date,accountId:row.account_id||'',type:row.entry_type,amount:Number(row.amount),note:row.note||''}}
function rowToReview(row:any):DailyReview{return {id:row.id,date:row.review_date,mood:row.mood||'Neutral',focus:Number(row.focus||3),sleep:Number(row.sleep||3),stress:Number(row.stress||3),confidence:Number(row.confidence||3),followedPlan:Boolean(row.followed_plan),overtraded:Boolean(row.overtraded),revengeTraded:Boolean(row.revenge_traded),movedStopLoss:Boolean(row.moved_stop_loss),positionSizeCorrect:Boolean(row.position_size_correct),wentWell:row.went_well||'',wentWrong:row.went_wrong||'',lesson:row.lesson||'',completed:Boolean(row.completed)}}
function reviewToRow(r:DailyReview,userId:string){return {id:r.id,user_id:userId,review_date:r.date,mood:r.mood,focus:r.focus,sleep:r.sleep,stress:r.stress,confidence:r.confidence,followed_plan:r.followedPlan,overtraded:r.overtraded,revenge_traded:r.revengeTraded,moved_stop_loss:r.movedStopLoss,position_size_correct:r.positionSizeCorrect,went_well:r.wentWell,went_wrong:r.wentWrong,lesson:r.lesson,completed:r.completed}}
...
export default function Home(){
  ...
  async function saveTrade(t:Trade,file?:File|null){
    if(demoMode){
      ...
      setTrades(prev=>editing?prev.map(x=>x.id===t.id?t:x):[t,...prev]);setModal(false);return
    }
    if(!user)return;
    setSyncing(true);setError('');
    if(file){
      if(file.size>5*1024*1024){setError('Screenshot must be 5 MB or smaller.');setSyncing(false);return}
      if(!['image/png','image/jpeg','image/webp'].includes(file.type)){setError('Only PNG, JPG and WebP screenshots are allowed.');setSyncing(false);return}
  ...
  async function saveReview(review:DailyReview){
    ...
    if(!user)return;setSyncing(true);
    const {data,error}=await supabase.from('daily_reviews').upsert(reviewToRow(review,user.id),{onConflict:'user_id,review_date'}).select().single();
    if(error)setError(error.message);else {const next=rowToReview(data);setReviews(prev=>prev.some(r=>r.date===next.date)?prev.map(r=>r.date===next.date?next:r):[next,...prev])}
    setSyncing(false)
  }
...
function Dashboard({stats,equity,directionData,instrumentData,onLog,onNav}:any){return <>
  ...
  <section className="panel"><div className="panelHead"><div><h3>Bull vs bear</h3><p>Which direction pays you better?</p></div></div>{directionData.map((d:any)=><div className={`directionCard ${d.name.toLowerCase()}`} key={d.name}><div><span>{d.name==='Bull'?'🐂':'🐻'} {d.name} trades</span><b>{money(d.value)}</b></div><div><strong>{d.winRate}%</strong><small>Win rate</small></div></div>)}</section></div>
...
function Reports({trades,stats,instrumentData}:any){const best=[...instrumentData].sort((a:any,b:any)=>b.pnl-a.pnl)[0];const report=`TickMint Monthly Review\n\nTrades: ${trades.length}\nNet P&L: ${money(stats.net)}\nWin rate: ${stats.winRate.toFixed(1)}%\nCurrent capital: ${money(stats.capital)}\nDiscipline: ${stats.discipline.toFixed(0)}%\nBest instrument: ${best?.name||'-'}\n`;return <><div className="pageHead"><div><span className="eyebrow">PERFORMANCE REVIEW</span><h1>Reports</h1><p>Turn raw trade history into a review you can act on.</p></div><button className="primaryLg" onClick={()=>downloadText(report,'tickmint-monthly-review.txt')}><Download size={18}/> Download report</button></div><div className="reportHero"><div><span className="pill">Monthly report ready</span><h2>Your capital grew, but discipline still explains the difference.</h2><p>Most profits came from rule-following Crude Oil and index setups.</p></div><div><span>Net P&L</span><b className="green">{money(stats.net)}</b></div></div><div className="reportCards">{[['Daily Review','Execution, emotion and one lesson from today.'],['Weekly Review','Best instrument, weakest setup and habit progress.'],['Monthly Review','Capital growth, drawdown, win rate and consistency.'],['Instrument Report','Detailed performance for one selected market.']].map(x=><button key={x[0]} onClick={()=>alert(`${x[0]}\n\n${x[1]}\n\nThis preview will become a generated PDF after backend integration.`)}><div><BookOpen/></div><h3>{x[0]}</h3><p>{x[1]}</p><span>Preview →</span></button>)}</div></>}
...
function TradeModal({trade,accounts,onClose,onSave}:{trade:Trade|null;accounts:TradingAccount[];onClose:()=>void;onSave:(t:Trade,file?:File|null)=>void}){
 ...
 <div className="directionPick"><button className={f.direction==='Bull'?'bull selected':'bull'} onClick={()=>update('direction','Bull')}>🐂 Bull <small>Long / Buy</small></button><button className={f.direction==='Bear'?'bear selected':'bear'} onClick={()=>update('direction','Bear')}>🐻 Bear <small>Short / Sell</small></button></div>
